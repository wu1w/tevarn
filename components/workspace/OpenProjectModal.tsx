...
export function OpenProjectModal() {
  ...
  const pickFolder = useCallback(async () => {
    ...
    try {
      ...
      setErr('当前环境无法弹出系统文件夹选择，请手动粘贴路径。');
    } catch (e) {
      setErr(e instanceof Error ? e.message : '选择失败');
    }
  }, []);
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-border-default bg-card-bg p-5 shadow-2xl">
        <h2 className="text-sm font-semibold text-foreground">选择项目文件夹</h2>
        <p className="mt-1.5 text-[12px] leading-relaxed text-foreground-muted">
          {uiMode === 'pro'
            ? '专业模式需要指定项目目录。文件树与终端将以该目录为工作根。'
            : '指定 AI 可以读写的工作文件夹。'}
        </p>
