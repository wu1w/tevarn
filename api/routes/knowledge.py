...
async def create_document(
    data: DocumentCreateWithContent,
    background: BackgroundTasks,
    current_user: Annotated[UserRead, Depends(get_current_user)],
    repo: Annotated[DocumentRepository, Depends(get_document_repo)],
):
    """创建文档；若有 content 则后台自动索引到向量库。"""
    payload = data.model_dump(exclude={"content"})
    payload["user_id"] = current_user.id
    content = (data.content or "").strip() or str((payload.get("meta") or {}).get("content") or "")
    if content:
