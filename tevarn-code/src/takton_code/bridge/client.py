"""HTTP client for Tevarn Desktop bridge (no-op when disabled)."""
...
class TevarnBridge:
    """HTTP bridge to Tevarn Desktop backend.
    ...
    async def agent_turn(self, req: Any) -> Any:
        ...
        from tevarn_code.bridge.protocol import AgentTurnRequest, AgentTurnResult
...
def build_bridge(config: BridgeConfig) -> TevarnBridge | NullBridge:
    if config.enabled:
        return TevarnBridge(config)
    return NullBridge()
