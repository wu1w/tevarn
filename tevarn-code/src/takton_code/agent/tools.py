...
from tevarn_code.compat.backend_core import DiffEngine
...
class ToolRuntime:
    ...
    def _register_builtin(self) -> None:
        ...
        self._register(
            ToolSpec(
                ...
                description="Search Tevarn Desktop RAG/knowledge base (requires bridge).",
                parameters={
                    "type": "object",
                    "properties": {
                        "query": {"type": "string"},
        ...
        self._register(
            ToolSpec(
                ...
                description="Invoke a Tevarn Desktop/MCP tool by name (requires bridge).",
                parameters={
                    "type": "object",
                    "properties": {
                        "name": {"type": "string"},
        ...
        self._register(
            ToolSpec(
                ...
                description="List skills from Tevarn Desktop bridge (requires bridge).",
                parameters={"type": "object", "properties": {}},
        ...
        self._register(
            ToolSpec(
                ...
                description="List MCP servers exposed by Tevarn Desktop bridge.",
                parameters={"type": "object", "properties": {}},
    ...
    async def list_desktop_skills(self, args: dict[str, Any]) -> str:
        if not self.bridge or not getattr(self.bridge, "enabled", False):
            return "ERROR: desktop bridge not enabled — start Tevarn Desktop or --bridge."
    ...
    async def list_desktop_mcp(self, args: dict[str, Any]) -> str:
        if not self.bridge or not getattr(self.bridge, "enabled", False):
            return "ERROR: desktop bridge not enabled — MCP lives on Tevarn Desktop."
        try:
    ...
    async def web_fetch(self, args: dict[str, Any]) -> str:
        ...
        try:
            ...
            req = urllib.request.Request(
                url,
                headers={"User-Agent": "TevarnCode/0.1 (+local-readonly-fetch)"},
                method="GET",
            )
    ...
    async def spawn_subagent(self, args: dict[str, Any]) -> str:
        ...
        from tevarn_code.agent.subagent import parse_spawn_args
