...
from tevarn_code.agent.parts import (
    Part,
    part_reasoning,
    part_step_finish,
    part_step_start,
...
from tevarn_code.agent.prompt import build_system_prompt
from tevarn_code.agent.refs import cycle_permission_mode, expand_at_refs
from tevarn_code.agent.tools import ToolRuntime
...
from tevarn_code.context.compressor import (
    ContextCompressor,
    TokenMeter,
    estimate_messages,
...
from tevarn_code.context.policy import (
    ThrashingGuard,
    build_context_meter,
...
from tevarn_code.llm.provider import LLMProvider, LLMResponse, collect_stream
from tevarn_code.project.binder import ProjectContext
...
class AgentRuntime:
    ...
    def _compose_extra(self, bridge_skills: str = "") -> str:
        ...
        if bool(getattr(self.settings_agent, "local_memory", False)):
            from tevarn_code.agent.memory_local import memory_prompt_block
    ...
    async def _rebuild_system_prompt(self, *, extra_agent: str = "") -> str:
        ...
        from tevarn_code.project.binder import bind_project
    ...
    async def _run_subagent(self, agent: str, prompt: str, max_iter: int) -> str:
        from tevarn_code.agent.subagent import run_subagent
    ...
    async def _run_turn_via_bridge(
        ...
        from tevarn_code.bridge.protocol import AgentTurnRequest
    ...
    async def _run_turn_unlocked(self, user_text: str, *, force_mode: str | None = None) -> TurnResult:
        ...
        from tevarn_code.compat.backend_core import build_user_content, content_for_storage
                ...
                if resp.tool_calls:
                    ...
                    for tc in resp.tool_calls:
                        ...
                        if isinstance(result, str) and len(result) > max_c:
                            from tevarn_code.context.compressor import CLEARED_TOOL_RESULT
    ...
    def focus_rewind_patch(self, which: str | int | None = None) -> str:
        ...
        from tevarn_code.compat.backend_core import format_rewind_side_panel
    ...
    async def list_hunks(self, path: str | None = None) -> str:
        from tevarn_code.compat.backend_core import hunks_summary, parse_unified_hunks
    ...
    async def apply_hunks_cmd(self, spec: str) -> str:
        ...
        from tevarn_code.compat.backend_core import parse_unified_hunks
    ...
    async def create_named_checkpoint(self, label: str = "manual") -> str:
        assert self.session_id and self.file_history
        from tevarn_code.agent.autoloop import _list_tracked_paths
    ...
    async def run_autoloop(
        ...
        *,
        ...
        from tevarn_code.agent.autoloop import run_autoloop
    ...
    async def _handle_slash(self, text: str) -> TurnResult | None:
        ...
        help_text = (
            ...
            "Ecosystem (skills/MCP/tools/RAG): Tevarn Desktop bridge --bridge"
        )
        ...
        if cmd in ("/auto-rules", "/autorules"):
            ...
            body += f"\nproject overlay: {self.project.root / '.tevarn' / 'auto_rules.toml'}"
            body += "\nenv: TEVARN_CODE_AUTO_RULES=<path>"
            return TurnResult(ok=True, final_text=body, mode=self.mode)
        ...
        if cmd == "/agent":
            from tevarn_code.agent.agents_fs import agents_summary, get_agent
        ...
        if cmd in ("/rules", "/reload-rules"):
            await self._rebuild_system_prompt()
            from tevarn_code.agent.auto_classify import clear_rules_cache, load_rules
        ...
        if cmd == "/memory":
            from tevarn_code.agent import memory_local as mem
        ...
        if cmd == "/inspect":
            ...
            info: dict[str, Any] = {
                ...
                "ecosystem": "Tevarn Desktop bridge (skills/MCP/tools/RAG) — not local copies",
        ...
        if cmd in ("/model", "/models"):
            ...
            from tevarn_code.config import apply_settings_json, load_settings
        ...
        if cmd == "/worktree":
            ...
            try:
                ...
                return TurnResult(
                    ...
                    final_text="",
                    mode=self.mode,
                    error="usage: /worktree [list|status]  (create via CLI: tevarn-code -w name)",
                )
