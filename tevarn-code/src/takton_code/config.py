"""Global configuration for Tevarn Code."""
...
def home_dir() -> Path:
    raw = os.environ.get("TEVARN_CODE_HOME", "").strip()
    if raw:
        return Path(raw).expanduser().resolve()
    return (Path.home() / ".tevarn-code").resolve()
...
class AgentSettings(BaseModel):
    ...
    # auto classifier rules path (optional override; else ~/.tevarn-code/auto_rules.toml)
    ...
    # opt-in local memory file ~/.tevarn-code/memory/MEMORY.md
...
class BridgeSettings(BaseModel):
    """Reserved: connect to Tevarn Desktop backend."""
...
def load_settings() -> Settings:
    ...
    env_map = {
        "TEVARN_CODE_BASE_URL": ("llm", "base_url"),
        "TEVARN_CODE_API_KEY": ("llm", "api_key"),
        "TEVARN_CODE_MODEL": ("llm", "model"),
        "TEVARN_CODE_CONTEXT_WINDOW": ("llm", "context_window"),
        "TEVARN_CODE_MAX_TOKENS": ("llm", "max_tokens"),
        "TEVARN_CODE_TEMPERATURE": ("llm", "temperature"),
        "TEVARN_CODE_COMPRESS_THRESHOLD": ("llm", "compress_threshold"),
        "TEVARN_CODE_BRIDGE_URL": ("bridge", "base_url"),
        "TEVARN_CODE_BRIDGE_TOKEN": ("bridge", "api_token"),
        "TEVARN_CODE_BRIDGE_ENABLED": ("bridge", "enabled"),
    }
...
def apply_settings_json(settings: Settings) -> Settings:
    """Merge ~/.tevarn-code/settings.json onto settings."""
    import json
