...
CODE_MD_TEMPLATE = """# Tevarn Code — project conventions
...
def bind_project(
    ...
    *,
    ...
    if worktree is not None and worktree is not False:
        from tevarn_code.compat.backend_core import WorktreeError, resolve_session_root
    ...
    for r in search_roots:
        ...
        if not ctx.code_md:
            ctx.code_md = _read_text(r / ".tevarn" / "CODE.md")
...
def init_project_files(root: Path, test_cmd: str | None = None, lint_cmd: str | None = None) -> Path:
    """Write .tevarn/CODE.md template if missing."""
    d = root / ".tevarn"
