"""Optional bridge: reuse Tevarn backend agent modules when monorepo path is available.

Isolated ``pip install tevarn-code`` keeps local implementations.
...
from tevarn_code.agent.permissions import (  # noqa: E402
    PermissionBroker,
    Reply,
)
from tevarn_code.agent.file_history import FileHistory  # noqa: E402
from tevarn_code.agent.redo import RedoStack, apply_redo_files, new_entry  # noqa: E402

try:
    from tevarn_code.agent.file_history import format_rewind_side_panel  # noqa: E402
...
try:
    ...
    from tevarn_code.agent.doom_loop import DoomLoopGuard
    from tevarn_code.agent.hunks import (
        apply_selected_hunks,
        hunks_summary,
        parse_unified_hunks,
    )
    from tevarn_code.agent.permissions import (
        PermissionGate,
        PermissionRule,
        rules_for_profile,
        TOOL_TO_KEY,
    )
    from tevarn_code.plan.gate import (
        PlanDocument,
        PlanGate,
        PlanState,
        PlanStep,
    ...
    from tevarn_code.agent.best_of_n import BonCandidate, score_candidate

    def pick_winner(cands):  # type: ignore
        from tevarn_code.agent.best_of_n import score_candidate as _sc
    ...
    from tevarn_code.agent.multimodal import (
        build_user_content,
        content_for_storage,
        find_image_paths,
    )
    from tevarn_code.diff.engine import DiffEngine, FileChange
    from tevarn_code.project.worktree import (
        WorktreeError,
        add_worktree,
        find_git_root,
        gc_worktrees,
...
def __getattr__(name: str):
    ...
    if name == "run_best_of_n":
        from tevarn_code.agent.best_of_n import run_best_of_n as _r
