"""System prompts for Tevarn Code — model-agnostic, open-source friendly.
...
CODE_SYSTEM = """\
You are Tevarn Code — an open-source, repo-native software engineering agent.
...
- Follow **project instruction files** when present (e.g. AGENTS.md, CODE.md, CLAUDE.md, .tevarn/*, README conventions). Project rules override default style preferences, not safety.
...
- desktop_* tools talk to Tevarn Desktop on localhost (skills/MCP/RAG). If bridge is off, continue with local tools only—do not invent bridge results.
...
def _locale_hint() -> str:
    ...
    loc = (os.environ.get("TEVARN_CODE_LOCALE") or os.environ.get("LANG") or "").strip()
    if not loc:
        return ""
