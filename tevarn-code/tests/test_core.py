...
from tevarn_code.context.compressor import ContextCompressor, TokenMeter, estimate_messages
...
async def test_session_store(tmp_path: Path):
    ...
    assert exp["format"] == "tevarn-code-session-v1"
    await s.close()


def test_parts_model():
    from tevarn_code.agent.parts import part_step_start, part_text, part_tool_start
...
def test_system_prompt_neutral_and_mode():
    from tevarn_code.agent.prompt import CODE_SYSTEM, build_system_prompt

    assert "Tevarn Code" in CODE_SYSTEM
