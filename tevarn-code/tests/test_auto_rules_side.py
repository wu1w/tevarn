...
from tevarn_code.agent.file_history import FileHistory, format_rewind_side_panel
from tevarn_code.session.store import SessionStore


def test_load_rules_default(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("TEVARN_CODE_HOME", str(tmp_path / "h"))
    clear_rules_cache()
...
def test_project_overlay_deny(tmp_path: Path, monkeypatch):
    ...
    clear_rules_cache()
    proj = tmp_path / "proj"
    (proj / ".tevarn").mkdir(parents=True)
    (proj / ".tevarn" / "auto_rules.toml").write_text(
        """
[[deny]]
on = "command"
...
""",
    ...
    assert c.decision == "deny"
    from tevarn_code.agent.auto_classify import rules_reload_info
...
async def test_rewind_includes_side_summary(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("TEVARN_CODE_HOME", str(tmp_path / "home"))
    db = tmp_path / "s.db"
    root = tmp_path / "proj"
    root.mkdir()
    f = root / "x.txt"
