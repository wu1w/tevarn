...
/// Resolve signing key: env TEVARN_PKG_SIGNING_KEY → derive JWT → insecure dev fallback.
pub fn resolve_signing_key_from_env() -> (Vec<u8>, String) {
    if let Ok(k) = std::env::var("TEVARN_PKG_SIGNING_KEY") {
        let t = k.trim();
    ...
    for env_name in ["TEVARN_JWT_SECRET", "JWT_SECRET", "TEVARN_API_KEY"] {
        if let Ok(k) = std::env::var(env_name) {
            let t = k.trim();
            if t.len() >= 16 {
                let mut h = Sha256::new();
                h.update(b"tevarn-pkg-signing-v2|");
                h.update(t.as_bytes());
    ...
    (
        b"tevarn-package-signing-v1-INSECURE-DEV-ONLY".to_vec(),
        "insecure_default".into(),
    )
}

/// `TEVARN_PKG_REQUIRE_SECURE=1|true|yes` forces production key policy.
pub fn resolve_require_secure_from_env() -> bool {
    match std::env::var("TEVARN_PKG_REQUIRE_SECURE") {
        Ok(v) => {
...
impl PackageManager {
    ...
    fn ensure_secure_key(&self, op: &str) -> Result<(), String> {
        if self.require_secure && self.is_insecure_default_key() {
            return Err(format!(
                "pkg:{op}:require_secure refused insecure_default key; set TEVARN_PKG_SIGNING_KEY or TEVARN_JWT_SECRET (>=16 chars)"
            ));
        }
        Ok(())
    }
    ...
    pub fn promote(&mut self, name: &str, force: bool) -> Result<InstalledPackage, String> {
        ...
        if self.is_insecure_default_key() {
            return Err(
                "cannot promote under insecure_default key; set TEVARN_PKG_SIGNING_KEY".into(),
            );
        }
    ...
    pub fn status(&self) -> Value {
        json!({
            ...
            "warning": if self.is_insecure_default_key() {
                "using insecure_default signing key; set TEVARN_PKG_SIGNING_KEY or TEVARN_JWT_SECRET"
            } else {
                ""
            },
        })
...
mod tests {
    ...
    fn insecure_default_never_verified() {
        let mut pm = PackageManager {
            ...
            signing_key: b"tevarn-package-signing-v1-INSECURE-DEV-ONLY".to_vec(),
