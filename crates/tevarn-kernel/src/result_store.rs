...
impl Default for ResultSpillStore {
    fn default() -> Self {
        ...
        // Override: TEVARN_RESULT_SPILL_THRESHOLD / TEVARN_RESULT_SPILL_PREVIEW.
        let thr = std::env::var("TEVARN_RESULT_SPILL_THRESHOLD")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(16_000);
        let preview = std::env::var("TEVARN_RESULT_SPILL_PREVIEW")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(8_000);
...
mod tests {
    ...
    fn spills_large_keeps_small() {
        let dir = env::temp_dir().join(format!("tevarn_spill_{}", short_id()));
