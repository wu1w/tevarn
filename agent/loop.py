...
class NexusAgentLoop:
    ...
    async def _get_rag_service(self):
        ...
        if self._rag_service is None:
            try:
                from backend.core.config import settings
                if not getattr(settings, "rag_enabled", True) and not getattr(settings, "qdrant_url", ""):
                    return None
    ...
    async def _inject_rag_context(
        ...
        *,
        ...
        """自动检索 RAG 知识库并将相关上下文注入到 messages"""
        from backend.core.config import settings as app_settings

        if not getattr(app_settings, "rag_enabled", True):
            return messages
    ...
    async def run(
        ...
        async with lock:
            return await self._run_locked(session_id, user_input, attachments, mode)
    ...
    async def _run_locked(
        ...
        try:
            ...
            messages, compress_meta = await compress_history_if_needed(
                messages, session_id=session_id, threshold=0.72
            )
            if compress_meta.get("compressed"):
                await self._push_status(
                    session_id,
                    "optimizing",
                    f"上下文已压缩（省略 {compress_meta.get('dropped_messages', 0)} 条历史）",
                )
        except Exception as e:
        ...
        for iteration in range(self.max_iterations):
            ...
            if tool_calls:
                ...
                for tc in tool_calls:
                    ...
                    try:
                        ...
                        if tool is not None:
                            ...
                            tool_result = await UnifiedToolRegistry.execute(tc.name, validated_args)
                            query = (
                ...
                if goal_mode:
                    ...
                    if (incomplete or no_plan) and goal_nudge_count < 8 and iteration < self.max_iterations - 1:
                        goal_nudge_count += 1
                        # 把当前文本当作中间思考，要求继续
                        if accumulated_content:
                            messages.append(
