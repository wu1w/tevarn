...
```text
python scripts/tevarn_sdk_pack.py ./my-agent              # 校验清单
python scripts/tevarn_sdk_pack.py ./my-agent --out dist/  # 校验并打 zip
python scripts/tevarn_eval.py                             # 固定评测
python scripts/ci_eval_gate.py                            # CI 门禁（可 --run）
```

打包产物：`{name}-{version}.tevarn-agent.zip`（单顶层目录，可被包市场安装流程消费）。
