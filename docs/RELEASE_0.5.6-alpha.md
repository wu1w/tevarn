...
- **打包布局下可找到 host 二进制**：`resources/takton-kernel-host/*`（Electron `extraResources`）不再只搜 `vendor/` / `target/`
- **启动 cwd 修复**：打包态不再用 asar 假路径 `resources/app` 作为 spawn cwd（Windows 上会导致 Host 直接起不来）
- **重启 Host 可用**：Electron 将 `TAKTON_KERNEL_HOST_BIN` / `TAKTON_RESOURCES_PATH` 注入后端，UI「重启 Host」taskkill 后可重新拉起

### 打包密钥卫生（勿把开发机 API/OAuth 打进发行包）
- Electron 后端环境 **剥离** 本机 `OPENAI_API_KEY` / `TAKTON_LLM_API_KEY` / OAuth token 等 shell 密钥
- 打包态设 `TAKTON_PACKAGED=1`，**不再加载 cwd `.env`**
...
- 若 0.5.5 包曾带出开发机 Key/OAuth：**轮换密钥**；本机若与安装版共用 `%APPDATA%\\takton`，旧 DB 配置仍会显示，属 userData 而非安装包
- 打 Windows 包前务必：
  ```powershell
