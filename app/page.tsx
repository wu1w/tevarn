...
export default function HomePage() {
  ...
  const { isConnected, isConnecting, sendMessage, sendStop, waitForConnection } = useWebSocket({
  ...
  const handleSend = useCallback(
      async (content: string, attachments: Attachment[] = [], mode: ChatMode = 'default') => {
        // D10 专业模式：强制项目文件夹
        ...
        addMessage(userMsg);
        const sent = sendMessage(content, attachments, mode);
        if (!sent) {
      ...
      {/* 顶部状态栏 */}
            <header className="flex items-center justify-between border-b border-border-subtle/50 bg-page-bg/80 backdrop-blur-xl px-5 py-2.5 sticky top-0 z-10">
              ...
              <div className="flex items-center gap-3">
                ...
                <kbd className="hidden sm:inline-flex items-center gap-1 rounded-md border border-border-subtle bg-page-bg px-2 py-1 text-[10px] text-foreground-dim font-mono">
                  <span className="text-xs">⌘</span>K
                </kbd>
                <div className="flex items-center gap-2">
                  <span className={`relative flex h-2 w-2 ${isConnected ? 'animate-pulse' : ''}`}>
                    <span className={`absolute inline-flex h-full w-full rounded-full opacity-75 ${isConnected ? 'bg-status-online animate-ping' : ''}`} />
                    <span className={`relative inline-flex h-2 w-2 rounded-full ${isConnected ? 'bg-status-online shadow-sm shadow-status-online/30' : 'bg-status-offline'}`} />
                  </span>
                  <span className="text-xs text-foreground-dim">{isConnected ? '已连接' : '未连接'}</span>
                </div>
                {isGeneratingImage && (
                  ...
                  <div className="relative flex min-h-0 flex-1 overflow-hidden">
                    <main className="chat-main-column">
                      ...
                      {!isConnected && !isConnecting && (
                        <div className="mx-3 mb-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
                          连接已断开，消息发送不可用。请检查网络或等待重连…
                        </div>
                      )}
                      <MessageInput
                        ...
                        disabled={isStreaming || isGeneratingImage || creatingSession || !isConnected}
                        placeholder={
                          creatingSession
                            ? '正在创建对话…'
                            : isStreaming
