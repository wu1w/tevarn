...
class WikiGraphRead(BaseModel):
    """一次性返回整个 Wiki 图谱"""

    entities: list[WikiEntityRead]
    relations: list[WikiRelationRead]
...
class WikiPreviewResult(BaseModel):
...
@router.get("/graph", response_model=WikiGraphRead)
async def get_graph(
    current_user: Annotated[UserRead, Depends(get_current_user)],
    entity_repo: Annotated[WikiEntityRepository, Depends(get_wiki_entity_repo)],
    relation_repo: Annotated[WikiRelationRepository, Depends(get_wiki_relation_repo)],
):
    """返回完整 Wiki 图谱（实体 + 关系），用于前端画布。"""
    entities, relations = await _load_graph(entity_repo, relation_repo)
    return WikiGraphRead(entities=entities, relations=relations)


async def _load_graph(
    entity_repo: WikiEntityRepository,
    relation_repo: WikiRelationRepository,
) -> tuple[list[Any], list[Any]]:
    """读取全部 Wiki 实体与关系。"""
    entities = await entity_repo.list_all(limit=2000)
    relations = await relation_repo.list_all(limit=5000)
...
async def delete_entity(
    ...
    if getattr(entity, "user_id", None) and entity.user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Access denied")
    # 级联删除关系
    await relation_repo.delete_by_entity(entity_id)
    success = await entity_repo.delete(entity_id)
    if not success:
        raise HTTPException(status_code=404, detail="Entity not found")
    return {"deleted": True}
...
async def delete_relation(
    ...
    rel = await relation_repo.get_by_id(relation_id)
    if rel is None:
        raise HTTPException(status_code=404, detail="Relation not found")
    success = await relation_repo.delete(relation_id)
    if not success:
        raise HTTPException(status_code=404, detail="Relation not found")
    return {"deleted": True}
...
_VALID_ENTITY_TYPES = set(WikiSchema.ENTITY_TYPES)
_VALID_RELATION_TYPES = set(WikiSchema.RELATION_TYPES)
...
def _build_extraction_prompt(raw_material: str, source: WikiImportSource) -> str:
    entity_types = "\n".join(f"- {k}: {v}" for k, v in WikiSchema.ENTITY_TYPE_LABELS.items())
    relation_types = "\n".join(f"- {k}: {v}" for k, v in WikiSchema.RELATION_TYPE_LABELS.items())
    source_hint = {
    ...
    return (
        ...
        "3. entity_type MUST be one of:\n{entity_types}\n"
        "4. relation_type MUST be one of:\n{relation_types}\n"
        ...
        raw_material=raw_material[:12000],
    )
...
@router.post("/import/preview", response_model=WikiPreviewResult)
async def preview_import(
    ...
    raw_material = await _build_raw_material(
        WikiImportRequest(
    ...
    extraction = await extract_and_merge(raw_material)
    return WikiPreviewResult(
...
async def ensure_wiki_seed() -> None:
    """启动时若 Wiki 为空，则写入预置通识种子。"""
    async with UnitOfWork() as uow:
        count = await uow.wiki_entities.count()
        if count > 0:
            return
        ...
        for item in WIKI_SEED_ENTITIES:
        ...
        for item in WIKI_SEED_RELATIONS:
            source = lookup.get(_normalize_name(item["source_name"]).lower())
            target = lookup.get(_normalize_name(item["target_name"]).lower())
            if not source or not target or source.id == target.id:
                continue
            existing_rel = await uow.wiki_relations.get_between(source.id, target.id)
            if existing_rel:
    ...
    logger.info("Wiki seed data initialized")
