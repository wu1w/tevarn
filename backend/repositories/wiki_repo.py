...
class WikiRelationRepository(BaseRepository):
    ...
    @abstractmethod
    async def list_all(self, limit: int = 2000) -> list[Any]:
        """列出所有关系"""
        raise NotImplementedError
...
class AsyncWikiEntityRepository(AsyncBaseRepository, WikiEntityRepository):
    ...
    async def count(self) -> int:
        ...
        try:
            ...
            result = await session.execute(select(func.count(WikiEntity.id)))
...
class AsyncWikiRelationRepository(AsyncBaseRepository, WikiRelationRepository):
    ...
    async def list_all(self, limit: int = 2000) -> list[WikiRelation]:
        session = await self._get_session()
        try:
            result = await session.execute(
                select(WikiRelation).limit(limit)
            )
            return list(result.scalars().all())
        finally:
            await self._close_session(session)
    ...
    async def delete_by_entity(self, entity_id: uuid.UUID) -> int:
        ...
        try:
            ...
            result = await session.execute(
                delete(WikiRelation).where(
                    (WikiRelation.source_id == entity_id) | (WikiRelation.target_id == entity_id)
                )
            )
            await self._maybe_commit(session)
