...
def test_detect_mcp_install_phrase():
    """口语「装下豆包搜索MCP：KEY」应走快路径，勿进 manage_mcp 空转。"""
    m = detect_config_intent(
        "装下豆包搜索MCP：IWimbUB9VSgFwoNpUmmTHeZZmrvMrarR"
    )
    assert m is not None
    assert m.kind == "mcp_key"
    assert m.payload["api_key"].startswith("IWim")
    assert "豆包" in str(m.payload.get("label") or "") or "doubao" in str(
        m.payload.get("label") or ""
    ).lower()
