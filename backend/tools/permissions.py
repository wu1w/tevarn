...
def host_data_roots() -> list[str]:
    ...
    ``~/.takton`` / ``~/.tevarn`` 与 ``%APPDATA%/{takton,tevarn}`` 是真实
    记忆/技能/日志落点；安装目录 ``Local\\Programs\\Tevarn`` 用于路径自检。
    ...
    try:
        ...
        hh = host_home()
        for sub in (".takton", ".tevarn"):
            try:
                roots.append(str((hh / sub).resolve()))
            except OSError:
                pass
    except Exception:
        for sub in (".takton", ".tevarn"):
            try:
                roots.append(str((Path.home() / sub).resolve()))
            except OSError:
                pass
    # 桌面端数据树 + 安装目录（Tevarn 产品与 Takton 开发双轨）
    for env_key in ("APPDATA", "LOCALAPPDATA"):
        base = (os.environ.get(env_key) or "").strip()
        if not base:
            continue
        for brand in ("takton", "tevarn", "Tevarn", "Takton"):
            try:
                roots.append(str((Path(base) / brand).resolve()))
            except OSError:
                continue
        if env_key == "LOCALAPPDATA":
            for brand in ("Programs/Tevarn", "Programs/Takton", "Programs/tevarn"):
                try:
                    p = Path(base) / brand.replace("/", os.sep)
                    if p.is_dir():
