...
def _product_display_name() -> str:
    """Tevarn desktop product vs Takton codebase naming."""
    import os

    for key in ("TEVARN_PRODUCT_NAME", "TAKTON_PRODUCT_NAME", "PRODUCT_NAME"):
        v = (os.environ.get(key) or "").strip()
        if v:
            return v
    # Heuristic: running under Tevarn install / APPDATA
    for key in ("LOCALAPPDATA", "APPDATA"):
        base = (os.environ.get(key) or "").replace("/", "\\").lower()
        if "tevarn" in base or "\\programs\\tevarn" in base:
            return "Tevarn"
    try:
        ...
        pass
    return "Tevarn"  # shipped desktop brand; open-source tree may still say Takton in code


DEFAULT_IDENTITY = (
    f"You are {_product_display_name()}, a local AI workstation agent. "
    "You are helpful, knowledgeable, and direct. You assist users with a wide "
    "range of tasks including answering questions, writing and editing code, "
    "analyzing information, creative work, and executing actions via your tools. "
    "You communicate clearly, admit uncertainty when appropriate, and prioritize "
    "being genuinely useful over being verbose. "
    "Follow TOOLS.md / workspace contract for tool, MCP, and path rules."
)
...
EVOLUTION_GUIDANCE = (
    ...
    "This product can draft skills from task experience (evolution tools). "
