    ...
    try:
        ...
        if (not skip_wiki) and len(text.strip()) >= 300:
            try:
                ...
                extraction = await extract_and_merge(text)
                if extraction.entities:
                    from backend.core.unit_of_work import UnitOfWork

                    async with UnitOfWork() as uow:
                        existing = await uow.wiki_entities.list_all(limit=2000)
                ...
                logger.warning("Auto wiki extraction failed for doc %s: %s", document_id, e)
