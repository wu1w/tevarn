...
try {
  ...
  Write-Info "API resolve failed ($($_.Exception.Message)); falling back to v0.3.3 asset names"
  $fallbackTag = if ($TagOverride) { $TagOverride } else { "v0.3.3" }
  $fallbackAsset = if ($AssetOverride) { $AssetOverride } else { "Takton-Setup-0.3.3.exe" }
