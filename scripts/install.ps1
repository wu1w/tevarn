...
try {
  ...
  Write-Info "API resolve failed ($($_.Exception.Message)); falling back to v0.2.8 asset names"
  $fallbackTag = if ($TagOverride) { $TagOverride } else { "v0.2.8" }
  $fallbackAsset = if ($AssetOverride) { $AssetOverride } else { "Takton-Setup-0.2.8.exe" }
