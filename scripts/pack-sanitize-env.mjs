...
for (const k of Object.keys(process.env)) {
  if (STRIP_EXACT.includes(k) || STRIP_RE.test(k)) {
    // Keep non-secret TEVARN_* operational vars (JWT is re-generated in Electron)
    if (
      k === 'TEVARN_JWT_SECRET' ||
      k === 'TEVARN_API_KEY' ||
      k === 'TEVARN_SETTINGS_ENCRYPTION_SALT' ||
      k === 'TEVARN_DESKTOP_PERMISSION_SECRET'
    ) {
...
delete process.env.TEVARN_ENV_FILE;
process.env.TEVARN_LOAD_DOTENV = '0';
process.env.TEVARN_PACKAGED = process.env.TEVARN_PACKAGED || '0';
...
function walk(dir, depth = 0) {
  ...
  for (const ent of entries) {
    ...
    if (ent.isDirectory()) {
      if (name === '.tevarn' || name === '.computers' || name === 'uploads') {
        offenders.push(full + '/');
        continue;
      }
      walk(full, depth + 1);
