# Build tevarn-kernel-host (Windows MSVC) and stage vendor copy for product discovery.
...
$cmd = "`"$cargo`" build -p tevarn-kernel-host $profileFlag"
...
if (Test-Path $vsDevCmd) {
    ...
    $args = @("build", "-p", "tevarn-kernel-host")
...
$built = if ($Release) {
    Join-Path $Root "target\release\tevarn-kernel-host.exe"
} else {
    Join-Path $Root "target\debug\tevarn-kernel-host.exe"
}
if (-not (Test-Path $built)) {
    # non-windows name
    $builtAlt = if ($Release) {
        Join-Path $Root "target\release\tevarn-kernel-host"
    } else {
        Join-Path $Root "target\debug\tevarn-kernel-host"
    }
...
$vendorDir = Join-Path $Root "vendor\tevarn-kernel-host"
