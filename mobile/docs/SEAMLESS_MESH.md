...
| 组件 | 作用 |
|------|------|
| `tevarn-tsnet` | 官方 tsnet userspace，PC 反代 / 手机 client-only |
...
```bash
cd sidecar/tsnet && go mod tidy && go build -o tevarn-tsnet .
export TEVARN_TSNET_BIN=$PWD/tevarn-tsnet
```
