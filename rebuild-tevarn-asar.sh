#!/usr/bin/env bash
# Tevarn asar 重新打包脚本
# 用途：当 Tevarn 前端代码修改后，重新构建并打包 app.asar
# 运行环境：Windows + MSYS/git-bash
# 要求：先关闭 Tevarn.exe（脚本会强制结束）

set -euo pipefail

# ---- 配置路径 ----
PROJECT_ROOT="/e/项目/tevarnl-0.1.0"
...
# ---- 步骤 1：关闭 Tevarn 进程 ----
log_info "步骤 1/10：关闭 Tevarn 进程..."
if taskkill.exe /F /IM Tevarn.exe 2>/dev/null || true; then
  sleep 2
  log_ok "Tevarn 进程已关闭"
else
  log_warn "未找到 Tevarn 进程，或已关闭"
fi
...
# ---- 步骤 9：启动 Tevarn.exe 验证（可选） ----
log_info "步骤 9/10：启动 Tevarn.exe 验证（可选）..."
TEVARN_EXE="$FRONTEND_DIR/release/win-unpacked/Tevarn.exe"
if [ -f "$TEVARN_EXE" ]; then
  log_ok "Tevarn.exe 已找到：$TEVARN_EXE"
  log_info "请手动启动 Tevarn.exe 验证窗口和后端是否正常工作"
else
  log_warn "未找到 Tevarn.exe：$TEVARN_EXE"
fi
