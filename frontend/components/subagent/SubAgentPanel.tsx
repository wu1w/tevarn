...
const TOOLSET_OPTIONS = [
  ...
  { value: 'browser', label: t('tools.type.browser'), icon: '' },
...
function ModelSelector({
  ...
  const groups = useMemo(() => {
    ...
    for (const item of inventory) {
      const group =
        item.status === 'active' || item.status === 'default'? t('subagent._e120')
          : item.status === 'fallback'? t('subagent._e121')
            : t('subagent._e122');
      (g[group] ||= []).push(item);
    }
    return g;
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute z-50 mt-1 max-h-64 w-full overflow-auto tk-card shadow-xl">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex shrink-0 items-center gap-0.5 opacity-80 group-hover:opacity-100">
              <button
                type="button"onClick={() => onToggle(agent.id, !agent.enabled)}
                className={`rounded-lg p-1.5 ${
                  agent.enabled
                    ? 'text-emerald-400 hover:bg-emerald-500/10': 'text-foreground-dim hover:bg-card-bg-hover'}`}
                title={agent.enabled ? t('cron.disabled') : t('channels.enable')}
              >
                <Power className="h-3.5 w-3.5" />
              </button>
              <button
                type="button"onClick={() => onEdit(agent)}
                className="rounded-lg p-1.5 text-foreground-dim hover:bg-card-bg-hover hover:text-foreground"title={t('memory.edit')}
              >
                <Edit3 className="h-3.5 w-3.5" />
              </button>
              {!agent.is_builtin && (
                <button
                  type="button"onClick={() => onDelete(agent.id)}
                  className="rounded-lg p-1.5 text-red-400/80 hover:bg-red-500/10"title={t('memory.delete')}
                >
...
function SubAgentFormDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm">
      <div className="max-h-[90vh] w-full max-w-lg overflow-auto tk-card rounded-2xl p-5 shadow-2xl">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {AVATAR_PRESETS.map((ic) => (
                <button
                  key={ic}
                  type="button"onClick={() => setForm({ ...form, icon: ic })}
                  className={`flex h-9 w-9 items-center justify-center rounded-xl border text-lg ${
                    form.icon === ic
                      ? 'border-brand-purple/40 bg-brand-purple/15': 'border-border-subtle hover:border-border-default'}`}
                >
                  {ic}
                </button>
              ))}
              <input
                ...
                className="h-9 w-16 rounded-xl border border-border-default bg-input-bg px-2 text-center text-sm"maxLength={8}
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-border-default bg-input-bg px-3.5 py-2.5 text-sm text-foreground placeholder:text-foreground-dim focus:border-brand-purple/40 focus:outline-none"placeholder={t('subagent._e77')}
            />
          </div>
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-border-default bg-input-bg px-3.5 py-2.5 text-sm text-foreground placeholder:text-foreground-dim focus:border-brand-purple/40 focus:outline-none"placeholder={t('subagent._e79')}
            />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-medium text-foreground-muted">
          ...
          <div>
            ...
            <textarea
              ...
              className="min-h-[120px] w-full resize-y rounded-xl border border-border-default bg-input-bg px-3.5 py-2.5 text-sm text-foreground placeholder:text-foreground-dim focus:border-brand-purple/40 focus:outline-none"placeholder={t('subagent._e80')}
            />
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {TOOLSET_OPTIONS.map((tool) => {
                ...
                return (
                  <button
                    key={tool.value}
                    type="button"onClick={() => toggleToolset(tool.value)}
                    className={`inline-flex items-center gap-1 rounded-lg border px-2.5 py-1 text-xs ${
                      on
                        ? 'border-brand-purple/30 bg-brand-purple/15 text-brand-cyan': 'border-border-subtle text-foreground-dim hover:border-border-default'}`}
                  >
            ...
            return (
              <label
                ...
                className={`flex cursor-pointer items-center gap-2 rounded-lg border px-2.5 py-2 transition-colors ${
                  on
                    ? 'border-brand-purple/35 bg-brand-purple/10': 'border-transparent hover:bg-card-bg-hover'}`}
              >
                <input
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-44 rounded-xl border border-border-default bg-input-bg py-1.5 pl-8 pr-3 text-sm text-foreground"placeholder={t('subagent._e84')}
            />
          </div>
          <button
            type="button"onClick={() => setFormDialog('new')}
            ...
            {inventory.slice(0, 16).map((item) => (
              <span
                ...
                className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[10px] ${
                  item.status === 'active'? 'bg-emerald-500/15 text-emerald-300': 'bg-card-bg-hover text-foreground-muted'}`}
              >
