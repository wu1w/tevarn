...
logger = logging.getLogger("tevarn-agent")
...
async def run_server(host: str, port: int, token: str, root: str, name: str):
    try:
        ...
        raise SystemExit(
            "tevarn-agent requires 'websockets'. pip install websockets"
        ) from e
    ...
    logger.info(
        "tevarn-agent listening ws://%s:%s root=%s name=%s",
        host,
        port,
        root,
        name,
    )
    # optional mDNS (_tevarn-agent._tcp.local)
    zc = info = None
    try:
        ...
        stype = "_tevarn-agent._tcp.local."
        info = ServiceInfo(
            stype,
            f"{name}.{stype}",
            addresses=addrs,
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(prog="tevarn-agent", description="Tevarn remote agent (L1)")
    parser.add_argument("--host", default="0.0.0.0")
    parser.add_argument("--port", type=int, default=DEFAULT_AGENT_PORT)
    parser.add_argument("--token", default=os.environ.get("TEVARN_AGENT_TOKEN") or "")
    parser.add_argument(
        "--root",
        default=os.environ.get("TEVARN_AGENT_ROOT") or str(Path.home() / "projects"),
    )
    parser.add_argument(
        "--name",
        default=os.environ.get("TEVARN_AGENT_NAME") or socket.gethostname(),
    )
    ...
    if not args.token:
        ...
        print(f"[tevarn-agent] generated token: {args.token}", file=sys.stderr)
    ...
    print(
        f"[tevarn-agent] name={args.name} ws://{args.host}:{args.port} root={args.root}",
        file=sys.stderr,
    )
    print(f"[tevarn-agent] token={args.token}", file=sys.stderr)
